...
patternMatching returns [PatternDeclaration patternRet]://TODO:cunstruct patterDeclaration node
    ...
    RPAR
    {
        Identifier id_1 = new Identifier($patternName.text);
        id_1.setLine($patternName.line);
        Identifier id_2 = new Identifier($targetVar.text);
        id_2.setLine($targetVar.line);
        $patternRet = new PatternDeclaration(id_1, id_2);
        $patternRet.setLine($pat.line);
    }
    (PATTERN_MATCHING_SEPARATOR c = condition{$patternRet.addCondition($c.conditionRet);}
